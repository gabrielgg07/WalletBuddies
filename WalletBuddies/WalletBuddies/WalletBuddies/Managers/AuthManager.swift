...
class AuthManager: ObservableObject {
    ...
    @Published var googleRestore: Bool = false {
        didSet {
            UserDefaults.standard.set(googleRestore, forKey: "googleRestore")
        }
    }
    ...
    func handleSignupSuccess(name: String, email: String, role:String) {
        self.name = name
        self.email = email
        self.role = role
    ...
    func handleLoginResponse(user: UserData) {
        self.name = user.name
        self.role = user.role
        handleSignupSuccess(name: user.name, email: user.email, role : user.role)
    }
...
//                self.gidToken = user.idToken
    ...
    func submitSignupForm(){
        ...
        URLSession.shared.dataTask(with: sendRequest){ data, response, error in
            ...
            do {
                ...
                if decoded.success{
                    DispatchQueue.main.async {
                        self.handleSignupSuccess(name: self.name, email: self.email, role: self.role)
                    }
                } else {
                    let errorMessage = decoded.message
                    print("❌ \(decoded.message)")
    ...
    //Delete an account as an admin
    func deleteAccountAsAdmin(accountEmail:String,complete:@escaping (Bool) -> Void) {
...
//                DispatchQueue.main.async { complete(false) }
    ...
    //Modify account privileges of existing accounts
    func modifyAccountAsAdmin(accountEmail:String,complete:@escaping (Bool) -> Void) {
        
        guard let url = URL(string: "\(BaseURL)/api/users/updateUserPrivilege") else {
            print("❌ Invalid URL")
            return
            
        }
        let payload : [String: Any] = [
            "requestRole" : self.role,
            "targetUserEmail" : accountEmail
        ]
        var request = URLRequest(url: url)
        ...
        URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if httpResponse.statusCode == 200 {
                print("✅ Account role successfully modified")
                // 🔹 Clear local user data
                DispatchQueue.main.async {
                complete(true)
                }
   ...
   //Login with existing google account already used to sign up to WalletBuddies
